...
Ahora, creamos un archivo de configuración base para git.
...
No es de mucha relevancia el usuario e email que escribamos aquí, funciona más
que nada como referencia para saber quién hizo un cambio. Por ejemplo, aquí
podría poner `name = compu1` para identificar que un cambio fue hecho desde la
computadora 1. El archivo debería verse así:

```toml {filename="~/.config/git/config"}
[user]
	email = miusuario@correo.com # no es necesario que sea un correo válido
	name = usuario               # no es necesario que sea el usuario de GitHub
```
...
## Agregar la llave en el host

Necesitamos agregar esta llave en la plataforma de preferencia.
...
Para agregarla en GitHub, pegamos la llave
en la pestaña [Ajustes > Llaves SSH y GPG](https://github.com/settings/keys).
